...
void LinkBoost::use(Game* game, Player* player, int row, int col) {
    Board* board = game->getBoard();
    Cell& cell = board->at(row, col);

    // handle no link
    if (cell.isEmpty()) throw invalid_argument("LinkBoost: no link at target cell");

    auto link = cell.getLink();

    // handle wrong owner
    if (link->getOwner() != player) throw invalid_argument("LinkBoost: can only boost your own link");
