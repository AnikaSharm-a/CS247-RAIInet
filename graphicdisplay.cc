...
GraphicDisplay::GraphicDisplay(int gridSize, int width, int height) : xw{width, height}, gridSize{gridSize}, gameRef(nullptr) {
    // calculate cell widths and heights, setup last drawn cache
    cellWidth = (width - 10) / gridSize;
    cellHeight = (height - 200) / gridSize;
...
void GraphicDisplay::drawCell(int r, int c, const Cell &cell, const Player *p1) const {
    ...
    if (!cell.isEmpty()) {
        ...
        // draw a smaller rectangle for the actual link, leaving a margin
        xw.fillRectangle(x + margin, y + margin, cellWidth - 2*margin, cellHeight - 2*margin, bgColor);
        ...
        // add an indicator if the link is boosted
        if (link->isBoosted()) {
...
// print function to display the game state
void GraphicDisplay::print(const Game &game, ostream &out) const {
    ...
    // build player1 info as a single string
    ...
    for (int r = 0; r < gridSize; ++r) {
        for (int c = 0; c < gridSize; ++c) {
            ...
            if (fogIt != foggedCells.end()) {
                // check if any fog is owned by player2 (for consistent player1 perspective)
                for (const auto& fog : fogIt->second.second) {
                    int fogOwnerId = fog.second;
                    if (fogOwnerId == 2) { // only show player2's fog since we are in player1's perspective
                        isFogged = true;
                        break;
                    }
                }
            ...
            // determine the drawn state based on cell content
            if (isFogged) {
                newState.symbol = '?';
            }
            else if (!cell.isEmpty()) { // if there is a link already
                auto link = cell.getLink();
                Player *owner = link->getOwner();
                bool visible = (owner == p1 || link->isRevealed());
            ...
            if (oldState.symbol != newState.symbol ||
                ...
                if (isFogged) { // draw the fog instead of the actual cells if there is fog
    ...
    // build player2 info as a single string
    ...
    // to display the links (if revealed, id otherwise ?)
    for (char id = 'A'; id <= 'H'; ++id) {
    ...
    // redraw player2 info only if changed
